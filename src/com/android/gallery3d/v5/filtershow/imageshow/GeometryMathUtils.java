...
import com.android.gallery3d.v5.filtershow.filters.FilterMirrorRepresentation.Mirror;
import com.android.gallery3d.v5.filtershow.filters.FilterRotateRepresentation.Rotation;
...
public final class GeometryMathUtils {
    ...
    public static void unpackGeometry(GeometryHolder out,
        ...
        for (FilterRepresentation r : geometry) {
            ...
            if(r.getSerializationName() == LewaFilterRotateRepresentation.SERIALIZATION_NAME){
            	LewaFilterRotateRepresentation lewaRotateRep = (LewaFilterRotateRepresentation)r;
            	out.rotation = lewaRotateRep.getRotateRepresentation().getRotation();
            	out.mirror = lewaRotateRep.getMirrorRepresentation().getMirror();
            }
    ...
    public static void initializeHolder(GeometryHolder outHolder,FilterRepresentation rotateRep,FilterRepresentation mirrorRep){
    	Collection<FilterRepresentation> geometry = MasterImage.getImage().getPreset()
                .getGeometryFilters();
    	replaceInstances(geometry, rotateRep);
    	replaceInstances(geometry,mirrorRep);
    	unpackGeometry(outHolder, geometry);
    }
